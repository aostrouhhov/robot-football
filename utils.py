...
from models import Robot, MovingObstacle

def calculate_new_position(vl, vr, x, y, theta, deltat):
        ...
        # Pure rotation motion
                ...
                xnew = x
                ynew = y
                thetanew = theta + ((vr - vl) * deltat / Robot.WIDTH)
        else:
                # Rotation and arc angle of general circular motion
                # Using equations given in Lecture 2
                R = Robot.WIDTH / 2.0 * (vr + vl) / (vr - vl)
                deltatheta = (vr - vl) * deltat / Robot.WIDTH
...
def calculate_closest_obstacle_distance(x, y, barriers):
